(function () {
  ...
  if (mode === 'ai') {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const response = await fetch(`/api/ai/${aiTask}`, {
          method: 'POST',
          body: formData
        });
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const createResponse = await fetch(`/api/${endpointBase}/${tool}`, { method: 'POST', body: formData });
      ...
      const url = completed.downloadUrl;
      download.href = url;
