...
const os = require('os');
const { enqueueJob, getJob, waitForRedis, redisConnectionFromEnv } = require('@propdfmate/queue');
...
const server = http.createServer(async (req, res) => {
  ...
  if ((req.url === '/health' || req.url === '/api/health') && req.method === 'GET') return sendJson(res, 200, { status: 'ok' });
...
(async () => {
  const redisOk = await waitForRedis(Number(process.env.REDIS_RETRY_MAX || 5), Number(process.env.REDIS_RETRY_DELAY_MS || 1000));
  const redisConn = redisConnectionFromEnv();
  if (redisOk) {
    console.log(`Redis reachable at ${redisConn.host}:${redisConn.port}`);
  } else {
    console.warn(`Redis not reachable after retries at ${redisConn.host}:${redisConn.port}; continuing with file-backed queue.`);
  }
